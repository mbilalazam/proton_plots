...
// MAIN
// ------------------------------------------------------------
void plot_dataMC_proton_length_GENIE()
{
	...
	const int    kNBins = 15;
	const double kXMin  = 3.0;
	const double kXMax  = 83.0;
	...
	// Load histograms
	// --------------------------------------------------------
	TH1D* hData_raw = (TH1D*)fData.Get("recoProtonLengthAll");
	TH1D* hMC_raw   = (TH1D*)fMC.Get("reco_matched_proton_length");
	...
	if (!grBand) {
		std::cerr << "GENIE length band not found\n";
		return;
	}
	...
	hData->GetYaxis()->SetRangeUser(0, 70);
	...
	c->SaveAs("proton_length_data_reco_GENIE_band.png");
}